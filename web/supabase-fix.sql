-- 既存テーブルを防音室用に修正するSQL
-- エラーが出た場合は、既存のテーブルを削除してから実行してください
...
-- 4. インデックスの作成
CREATE INDEX idx_reservations_date_store ON reservations(reservation_date, store_id);
CREATE INDEX idx_reservations_customer ON reservations(customer_id);
CREATE INDEX idx_customers_student_id ON customers(student_id);
